...
public class select_boss_popup : MonoBehaviour {

    public static bool check_popup_window = false;
    void Start()
    {
        setDisable();
    }
    ...
    void open()
    {
        ...
        {
            print("boss popup window open!!");
            // 먼저 버튼이 enable하만큼 충분한 gemstone이 있는지 체크함.
            boss_button_mgr.check_gemstone_for_boss_button();
            // 그러고 나서 다시 조건이 맞는지 체크후 enable or not.
            boss_button_mgr.check_boss_button_enable_or_not();
        }
        init();
